...
async function checkSchema() {
  console.log('🔍 Auditando tabla "resultados"...');
  const { error } = await supabase.rpc('exec_sql', { 
    query_text: "ALTER TABLE resultados ADD COLUMN IF NOT EXISTS sucursal TEXT;" 
  });

  if (error) {
    console.error('❌ Error al auditando:', error.message);
  } else {
    console.log('✅ Columna "sucursal" verificada/creada en la tabla resultados.');
  }
}
