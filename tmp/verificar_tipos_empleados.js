...
async function checkCols() {
  const sql = "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'empleados' AND column_name = 'id';";
  const { data, error } = await supabase.rpc('exec_sql', { query_text: sql });
  if (error) console.error(error.message);
  else console.log('📂 Columna empleados.id es tipo:', data[0].data_type);
}
