...
const tablesSQL = `
-- 1. Crear Tablas de Mantenimiento
...
-- 3. Limpiar y sembrar datos iniciales
...
async function runMigration() {
  console.log('🚀 Iniciando creación de tablas e importación de datos semilla de mantenimiento...');
  const { data, error } = await supabase.rpc('exec_sql', { query_text: tablesSQL });

  if (error) {
    console.error('❌ Error ejecutando migración de tablas:', error.message);
    console.error('Detalles:', error);
    process.exit(1);
  } else {
    console.log('✅ Tablas creadas y datos semilla sembrados exitosamente!');
    
    // Configurar Storage Bucket a través de la API de Clientes de Supabase
    console.log('📦 Configurando el bucket "mantenimiento-evidencia"...');
    const { data: bucketData, error: bucketError } = await supabase.storage.createBucket('mantenimiento-evidencia', {
      public: true,
    ...
    if (bucketError) {
      if (bucketError.message.includes('already exists') || bucketError.message.includes('duplicate key value')) {
        console.log('ℹ️ El bucket "mantenimiento-evidencia" ya existe.');
      } else {
        console.warn('⚠️ Advertencia configurando bucket:', bucketError.message);
      }
    } else {
      console.log('✅ Bucket "mantenimiento-evidencia" creado exitosamente!');
    }

    console.log('📊 Verificando conteos en la base de datos...');
    const verifySQL = `
      SELECT 
    ...
    `;
    const verifyRes = await supabase.rpc('exec_sql', { query_text: verifySQL });
    if (verifyRes.error) {
      console.error('⚠️ Error al verificar conteos:', verifyRes.error.message);
    } else {
      console.log('📈 Conteo final de registros:', verifyRes.data);
    }
    process.exit(0);
  }
}
